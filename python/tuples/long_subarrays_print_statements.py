...
def count_long_subarrays(A):
    ...
    for i in range(0, n - 1):
        ...
        # if the number following the previous one is bigger, append it to temp and increment num_subarrays
            ...
            temp.append(A[i])
            print("i=", i)
            print("A[i] > A[i - 1], so temp is", temp, "\n")
        # if not
        elif A[i] <= A[i - 1]:
            print("i=", i)
            print("A[i] < A[i - 1], so temp stays as", temp, "& len(temp) is", len(temp))
            ...
            if len(temp) >= size_first_sequence:
                num_subarrays+=1
                print("size_first_sequence is", size_first_sequence, "and num_subarrays is", num_subarrays)
            else:
                pass
            # clear the temp list
            temp.clear()
    ...
    for i in range(n - 2, n - 1):
        if A[i + 1] > A[i]:
            ...
            print("i=", i)
            print("A[i + 1] < A[i], so temp stays as", temp, "& len(temp) is", len(temp))
            # if len(temp) > size_subarray one, store that size for future reference so that all future subarrays are that length or longer
            if len(temp) >= size_first_sequence:
                num_subarrays+=1
                print("size_first_sequence is", size_first_sequence, "and num_subarrays is", num_subarrays)
